...
Local links may target the dataset directory in the optional sibling
`0_demo_TurbulentCombustion` tree. That historical demo tree is local-only and
is not shipped with this branch. The files do not contain complete modern
provenance, units, or checksums; this limitation is recorded by validation
rather than guessed.

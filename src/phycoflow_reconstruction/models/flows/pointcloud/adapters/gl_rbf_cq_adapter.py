"""Project adapter for the dataset-independent GL-RBF/CQ tensor core.

The core package owns the model implementation and tensor-level RF helpers.
...
from ..core.portable_core import PointCloudFFM as PortablePointCloudFFM
from ..core.priors import IIDGaussianPrior as PortableIIDGaussianPrior
from ..core.priors import RFFGaussianPrior as PortableRFFGaussianPrior
from ..runtime import (
    ModelEMA,
...
def _portable_config(config: Mapping[str, Any], *, coordinate_dim: int) -> dict[str, Any]:
    """Map downstream spelling to the frozen portable public configuration."""

    resolved = {
        key: value for key, value in config.items() if key not in {"name", "model_name", "backbone"}
    }
    for source, target in _ALIAS_KEYS.items():
        if source in resolved:
            if target in resolved and resolved[target] != resolved[source]:
                raise ValueError(f"conflicting GL_rbf_CQ settings {source!r} and {target!r}")
...
class GLRbfCQ(PortablePointCloudFFM):
    ...
    def __init__(
        ...
        **config: Any,
        ...
        self.obs_consistency_mode = str(config.get("obs_consistency_mode", "endpoint_smooth"))
        self.obs_consistency_strength = float(config.get("obs_consistency_strength", 1.0))
        ...
        self.obs_consistency_final_clamp = bool(config.get("obs_consistency_final_clamp", True))
    ...
    def _validate_batch(self, batch: ObservationBatch, *, require_target: bool) -> None:
        batch.validate()
    ...
    def reconstruct(
        ...
        **kwargs: Any,
        ...
        all_observations_in_query = clamp_mask is not None and bool(
            torch.equal(clamp_mask, batch.obs_valid_mask)
        )
        if (
        ...
        config = ReconstructionConfig(
            ...
            obs_consistency_sigma=float(get("obs_consistency_sigma", self.obs_consistency_sigma)),
            ...
            cache_level=str(get("reconstruction_cache_level", self.reconstruction_cache_level)),
        )
        ...
        if (
            ...
            for batch_index in range(prediction.shape[0]):
                ...
                prediction[
                    batch_index,
                    local_indices[batch_index, valid],
                    batch.obs_field_ids[batch_index, valid],

...
`configs/readiness/ABC_sliced_persistence_formal_5000ep_gpu1.yaml` is the fresh A+B+C formal profile: 5000 configured epochs, optimizer batch size 32, training fraction 0.15, and 16 coherence samples per update on GPU 1. This gives 38 updates per reported epoch; the initial batch-16 attempt gave 75. It retains the source `last.pt`, topology-with-fidelity checkpoint selection, 32-sample validation panel, and 200-epoch reconstruction interval. Earlier A+B+C test runs and the superseded batch-16 formal attempt are archived under `runs/bk/`.
